    ...
    public function iniciar_sesion()
    {
        ...
        if ($data != null) {
            ...
            if ($data->estatus_usuario == '1') {
                if ($data->id_rol == ROL_ADMINISTRADOR['clave'] || $data->id_rol == ROL_OPERADOR['clave']) {
                ...
                } else if ($data->id_rol == ROL_CLIENTE['clave']) {
                    $session->set("pagina_actual", "Home");
                    $session->set("tarea_actual", "Home");
                    helper('message');
                    make_message(INFO_ALERT, 'Al Sistema ' . $data->usuario, 'Bienvenido');
